...
TEST_F(ViewStyleFixture, PlacementRowHasASingleColumnNoLayerAndIsOneShadeLighterThanBoundary)
{
    ...
    const auto &placement_row = view_layers.rows().at(2);
    ASSERT_EQ(placement_row.columns.size(), 1u);
    EXPECT_EQ(placement_row.columns[0].purpose, ViewLayerPurpose::PLACEMENT);
    EXPECT_EQ(placement_row.columns[0].id, view_layers.placement_view_layer());

    const ViewLayerData *placement = view_layers.get(view_layers.placement_view_layer());
    ASSERT_NE(placement, nullptr);
    EXPECT_FALSE(placement->layer.valid());

    const ViewLayerData *boundary = view_layers.get(view_layers.boundary_view_layer());
    ASSERT_NE(boundary, nullptr);
    EXPECT_EQ(placement->style.outline_color.r, 220);
    EXPECT_EQ(placement->style.outline_color.g, 220);
    EXPECT_EQ(placement->style.outline_color.b, 220);
    EXPECT_GT(placement->style.outline_color.r, boundary->style.outline_color.r) << "should be lighter than boundary_style()'s own 160";
    EXPECT_FALSE(placement->style.dashed);
    EXPECT_EQ(placement->style.fill_color.a, 0);
}
