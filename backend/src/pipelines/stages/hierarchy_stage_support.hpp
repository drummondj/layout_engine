...
namespace le
{
    ...
    inline sk_sp<SkPicture> record_local_picture(
        ...
        // BUGS_AND_ENHANCEMENTS.md E31's zoom/pan-tick follow-up -
        // nullptr (every caller except build_top_layout_picture) keeps
        // the original "cull_scene fully encloses content_bbox, nothing
        // real gets culled" behavior below, required for a recursively-
        // cached node's own picture (HierarchyAbstractLeafStage/
        // HierarchyLayoutNodeStage): that picture is shared by every
        // placement referencing it, at any pan, so it must contain its
        // OWN full content regardless of what's on-screen for any one
        // viewer right now. build_top_layout_picture's own picture is
        // never cached or reused across a pan/scale change at all
        // (top_layout_picture_stage_'s own key includes viewport_version,
        // so it's rebuilt from scratch on every view change anyway) - so
        // for that one caller specifically, a real, tighter viewport
        // (the same cull_rect visible_top_level_placements already
        // queries the placement spatial index with) is passed here to
        // ALSO cull the Layout's own direct content (dbu_shapes) to what
        // this one frame can actually show, instead of transforming and
        // drawing all of it unconditionally - measured on a real
        // 372,096-placement tiled fixture: 4,306,612 routed-path
        // segments (dbu_shapes), of which 4,303,234 (99.9%) still passed
        // through unfiltered on every single call even after the
        // scale-only cache-trigger fix below, since cull_scene's own
        // viewport never actually excluded anything by design.
        const Rect *real_viewport_cull_rect = nullptr)
    {
        ...
        // A throwaway Scene giving ViewportFilterStage the viewport rect
        // it culls against - by default (real_viewport_cull_rect ==
        // nullptr) one that fully encloses content_bbox at `scale`, so
        // nothing real gets culled (see this function's own
        // real_viewport_cull_rect parameter doc comment above for when
        // and why that's required); when a real rect IS given, use it
        // directly instead - a genuinely tighter viewport, so shapes
        // outside it really do get dropped. +2px margin absorbs
        // floating-point rounding at the exact boundary either way.
        Scene cull_scene;
        cull_scene.set_scale(scale);
        const Point cull_ll = real_viewport_cull_rect ? real_viewport_cull_rect->ll : content_bbox.ll;
        const Point cull_ur = real_viewport_cull_rect ? real_viewport_cull_rect->ur : content_bbox.ur;
        cull_scene.set_pan(cull_ll);
        const double width_dbu = static_cast<double>(cull_ur.x - cull_ll.x);
        const double height_dbu = static_cast<double>(cull_ur.y - cull_ll.y);
        cull_scene.set_viewport_size(static_cast<int>(width_dbu * scale) + 2, static_cast<int>(height_dbu * scale) + 2);

        // Viewport culling uses cull_scene (above) - but layer
        // VISIBILITY must use the real `scene`, not cull_scene:
        // FilterByLayerVisibilityStage checks Scene::is_view_layer_visible
        // against the user's actual layer show/hide toggles, which a
        // fresh cull_scene has no knowledge of (every layer defaults to
        // visible on a just-constructed Scene) - using cull_scene here
        // would silently ignore every real visibility toggle for
        // anything drawn inside a cached instance picture. Two different
        // Scenes for two different purposes, not a copy-paste slip -
        // this is exactly why the chain below still takes two separate
        // PipelineOptions rather than one shared set.
        //
        // The cache-invalidation trigger fed into viewport_version here
        // used to be the REAL scene's own viewport_version() - which
        // set_pan/set_viewport_size bump too, not just set_scale
        // (scene.hpp) - even though, in the default (nullptr) case,
        // cull_scene's own viewport always fully encloses content_bbox
        // regardless of the real scene's pan, so ViewportFilterStage's
        // actual OUTPUT never depended on pan at all, only on scale
        // (min_visible_dbu = 1.0/scale). That mismatch forced a full
        // re-cull of every one of the Layout's own direct shapes on
        // every single call, real or not. Folding scale (and, when a
        // real cull rect is given, its own bounds too - a genuine
        // dependency now that the viewport can actually differ between
        // calls) via combine_versions is the precise, minimal trigger
        // either way: unchanged across a call where nothing relevant
        // moved, changed the instant something that could affect the
        // actual filtered output does.
        uint64_t cull_trigger = std::bit_cast<uint64_t>(scale);
        if (real_viewport_cull_rect)
            cull_trigger = combine_versions(cull_trigger,
                                             std::bit_cast<uint64_t>(real_viewport_cull_rect->ll.x), std::bit_cast<uint64_t>(real_viewport_cull_rect->ll.y),
                                             std::bit_cast<uint64_t>(real_viewport_cull_rect->ur.x), std::bit_cast<uint64_t>(real_viewport_cull_rect->ur.y));

        PipelineOptions viewport_options;
        viewport_options.ctx.scene = &cull_scene;
        viewport_options.viewport.viewport_version = cull_trigger;
