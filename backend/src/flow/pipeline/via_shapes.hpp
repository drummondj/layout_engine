...
namespace le::flow
{
    /// @brief Fresh copy of src/pipeline/stages/via_shapes.hpp's
    /// append_via_shapes, ported verbatim into le::flow rather than
    /// #include-d, so this experiment (backend/TASKFLOW_EXPERIMENT.md)
    /// stays fully self-contained under src/flow - no coupling into
    /// src/pipeline. See the original's own doc comment for the full
    /// via-resolution semantics (this is that same logic, unchanged).
