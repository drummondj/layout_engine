...
- [x] E10. Max CPU control. When running on machines with 128+ CPUs I need to control the maximum threads avaialbe to layout engine's backend. So I need an arena with a user settable concurrency flag. Minimum should be 2, one for the TCL interface and one for ImGUI? With a default of 8.
